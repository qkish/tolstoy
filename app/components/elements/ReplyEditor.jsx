...
class ReplyEditor extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (process.env.BROWSER) {
      ...
      if (tp.body.value !== np.body.value ||
        ...
        (np.filemeta && tp.filemeta.value !== np.filemeta.value)) { // also prevents saving after parent deletes this information
  ...
  render() {
    ...
    const postLabel = username ? <Tooltip
      t={translate('post_as') + ' “' + username + '”'}>{translate('post')}</Tooltip> : translate('post')
    ...
    let show_youtube_insert = <div className="">
      ...
      <form><input type="text" className="ReplyEditorShort__input-youtube"
                   name="youtube" ref="youtubeEditRef"/>
        <div className={youtubeError}>Ошибка, неверная ссылка</div>
        <input type="submit"
               className="ReplyEditorShort__submit-youtube ReplyEditorShort__buttons-submit button"
               value="Вставить" onClick={this.insertYoutube}/>
        <input type="button"
               className="ReplyEditorShort__submit-youtube secondary hollow button no-border ReplyEditorShort__buttons-submit ReplyEditor__buttons-cancel"
               value="Отмена" onClick={this.hideYoutube}/>
    ...
    let youtubeModal = <Reveal
      show={show_youtube_bool}>{show_youtube_insert}</Reveal>
        ...
        <div className="column small-12">
          <form className={vframe_class}
                ...
                onSubmit={handleSubmit(data => {
                  const loadingCallback = () => this.setState({
                    loading: true,
                    postError: undefined
                  })
                  this.setState({
                  ...
                  reply({
                    ...Object.assign({}, data, {body: `${data.body} ${imageAdded || ''} ${youtubeAdded || ''}`}), ...replyParams,
                    loadingCallback
                  })
                })}
                onChange={() => {
          ...
          >
            <div className={vframe_section_shrink_class}>
              {isStory && <span>
                                <input type="text"  {...cleanReduxInput(title)}
                                       onChange={onTitleChange}
                                       onTouchStart={this.handleOnTitleFocus}
                                       disabled={loading}
                                       placeholder={translate('reporttitle')}
                                       autoComplete="off" ref="titleRef"
                                       tabIndex={1}
                                       onMouseDown={this.handleOnTitleFocus}
                                       onBlur={this.handleOnTitleBlur}
                                       className='ReplyEditorShort__titleVisible'/>
                            </span>}
            </div>
            
            <div
              className={'ReplyEditorShort__body ' + (rte ? `rte ${vframe_section_class}` : vframe_section_shrink_class)}
              onClick={this.focus}>
              
              <textarea {...cleanReduxInput(body)} disabled={loading}
                        rows={isStory ? 1 : 3}
                        placeholder={translate(isStory ? 'write_your_story' : 'reply')}
                        autoComplete="off" ref="postRef" tabIndex={2}
                        onMouseDown={this.handleOnFocus}
                        onTouchStart={this.handleOnFocus}
                        onBlur={this.handleOnBlur} className='expanded-area'/>
            </div>
            {isEdit && isStory ?
              <input type="number" {...cleanReduxInput(money)}
                     onChange={onMoneyChange} disabled={loading}
                     placeholder={translate('money_for_day')} autoComplete="off"
                     ref="moneyRef" tabIndex={7}
                     onMouseDown={this.handleOnMoneyFocus}
                     onTouchStart={this.handleOnMoneyFocus}
                     onBlur={this.handleOnMoneyBlur}
                     className='ReplyEditorShort__moneyVisible'/> : ''}
            {isEdit && isStory ?
              <input type="hidden" {...cleanReduxInput(filemeta)}
                     value={this.state.fileState}/> : ''}
            
            <div className={vframe_section_shrink_class}>
              <div
                className="error">{body.touched && body.error && body.error !== 'Required' && body.error}</div>
            </div>
            ...
            {hasCategory && <span>
                                <CategorySelector {...category}
                                                  disabled={loading}
                                                  isEdit={isEdit} tabIndex={3}/>

                            </span>}
            
            <div className={vframe_section_shrink_class}>
              {postError &&
              <div className="error">{translateError(postError)}</div>}
            </div>
            ...
            <div className={(vframe_section_shrink_class) + " uncovered"}>
              {!loading && <button type="submit"
                className={"button ReplyEditorShort__buttons-submit uncoveder"}
                disabled={submitting || invalid}
                tabIndex={4}>{isEdit ? translate('update_post') : postLabel}</button>}
              {loading && <span><br /><LoadingIndicator type="circle"/></span>}
              &nbsp; {!loading && this.props.onCancel &&
            <button type="button"
                    className="secondary hollow button no-border ReplyEditorShort__buttons-submit ReplyEditor__buttons-cancel"
                    tabIndex={5} onClick={(e) => {
              e.preventDefault();
              onCancel()
            }}>{translate("cancel")}</button>
            }
              
              <div className={'ReplyEditorShort__buttons-add ' + loadingHide}
                   onMouseDown={this.handleOnButtonsFocus}
                   onTouchStart={this.handleOnButtonsFocus}
                   onClick={this.handleOnButtonsFocus}>
                <ul>
                  <li>
                    <Upload
                      component='label'
                  ...
                  <li><a href="#"
                         className="ReplyEditorShort__buttons-add-video"
                         onClick={this.handleUploadYoutube}></a></li>
                  <li>
                    <Upload
                      component='label'
                      action='/api/v1/upload'
              ...
              {isStory && !isEdit && <div className="float-right">
                ...
                <input type="hidden" onChange={this.toggleAllSteemPower}
                       checked={allSteemPower}/>
                
                <input type="hidden" {...cleanReduxInput(autoVote)}
...
export default formId => reduxForm(
  ...
  (dispatch, ownProps) => ({
    ...
    reply: ({
      category, title, body, money, filemeta, author, permlink, parent_author, parent_permlink,
      ...
      if (meta && meta.tags && meta.tags.length > 5) {
        ...
        errorCallback(translate('use_limited_amount_of_tags', {
