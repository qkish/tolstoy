...
class User extends Component {
	...
	render() {
		...
		if (this.props.postdate) postDateTime =
			<TimeAgoWrapper date={this.props.postdate} className="updated"/>
		
		// try to extract image url from users metaData
		
		// try to extract image url from users metaData
		try {
			let parseRes = JSON.parse(account.json_metadata)
		...
		return (
			<div className="User__wrap">
				<div className="Author__avatar_wrapper">
					<div className="User">
						{process.env.BROWSER ?
							url ? <a href={linkFromProps || '/@' + username.name}> <img
								src={_urls.proxyImage(url || '')} onError={this.onError}/></a> :
								<a href={'/@' + username.name}>
									<div className="User__defaultAva"></div>
								</a> :
							<LoadingIndicator type="circle" inline/>}
					</div>
				</div>
				...
				<span className="Author">
					<span itemProp="author" itemScope
					    ...
					    <Link
					      to={linkFromProps || '/@' + username.name}>{nameFromProps || fullName}</Link>
					</span>
					{postDateTime}
				</span>
